...
from python.analysis import Fitting, vector, Tags, Slicing, Processing
import python.analysis.cross_section as xsec
import python.analysis.EnergyTools as etools
from python.analysis.Master import LoadObject, SaveObject, Data

@dataclass
class AnalysisInputBase:
    # masks
    classification_info : dict[np.ndarray]
    inclusive_process : np.ndarray
    ...
    def region_labels(self):
        return list(self.classification_info.keys())
    ...
    def __len__(self):
        for o in ["outside_tpc_reco", "outside_tpc_true",
                  "track_length_reco", "KE_int_reco", "KE_init_reco",
                  "mean_track_score", "track_length_true",
                  "KE_int_true", "KE_init_true", "weights"]:
    ...
    def CreateAnalysisInputToy(cls, toy) -> "AnalysisInput":
        ...
        """
        inclusive_events = np.array(
            (toy.df.inclusive_process != "decay").values)

        regions = {
            k : np.array(v.values) for k, v in toy.reco_regions.items()}
        process = {
            k : np.array(v.values) for k, v in toy.truth_regions.items()}
    ...
    def CreateAnalysisInputNtuple(
        ...
        Args:
            ...
            upstream_energy_loss_params (dict): upstream energy loss
                correction
            reco_regions (dict[np.ndarray]): reco region masks
            true_regions (dict[np.ndarray], optional): true process
                masks. Defaults to None.
            mc_reweight_params (dict, optional): mc reweight
        ...
        if mc_reweight_params is not None:
            weights = xsec.RatioWeights(
                events.recoParticles.beam_inst_P, "gaussian",
                mc_reweight_params, mc_reweight_stength)
        else:
            weights = None

        reco_KE_inst = etools.KE(events.recoParticles.beam_inst_P,
                          Particle.from_pdgid(211).mass)
        reco_upstream_loss = etools.UpstreamEnergyLoss(
    ...
    @classmethod
    def Concatenate(cls, ais : list["AnalysisInput"]):
        fields = Processing.MergeOutputs([vars(a) for a in ais if a is not None])
        return cls(**fields)


    def CreateTrainTestSamples(
            self,
            seed : int, train_fraction : float = None) -> dict:
        """ Split analysis input into two samples

        Args:
            seed (int): seed for random permutation
            train_fraction (float, optional): fraction of events to
            assign to train, if None, sample is split 50/50. Defaults
            to None.
        ...
        return {"train" : AnalysisInput(**train),
                "test" : AnalysisInput(**test)}

@dataclass
class AnalysisInput(AnalysisInputBase):

    @property
    def regions(self):
        return self.classification_info
    ...
    def NInteract(
        ...
        """ Calculate exclusive interaction histogram using the energy
        slice method.
        ...
        n_interact = Slicing.EnergySlice.CountingExperiment(
            KE_int[mask], KE_init[mask], outside_tpc[mask],
            process[mask], energy_slice, interact_only = True,
            weights = weights[mask] if weights is not None else weights)
        return n_interact
    ...
    def CreateHistograms(
        ...
        n_initial, n_interact_inelastic, n_interact_exclusive, n_incident = (
        ...
        output = {
            ...
            "int_ex" : n_interact_exclusive, "inc" : n_incident}
        return output

@dataclass
class AnalysisInputGNN(AnalysisInputBase):
    ...
    def CreateAnalysisInputToy(cls, toy) -> "AnalysisInput":
        ...
        raise NotImplementedError(
            "Not able to generate GNN scores from toys.")
        # inclusive_events = np.array(
        #     (toy.df.inclusive_process != "decay").values)
    ...
    def CreateAnalysisInputNtuple(
        ...
        Args:
            ...
            upstream_energy_loss_params (dict): upstream energy loss
                correction
            gnn_predictions (np.ndarray): `(N, D)` array with the `D`
                GNN score predictions for each of the `N` events.
            prediction_labels (list[str]): List of `D` strings
                labelling the `D` score predictions in the GNN ouput.
            true_regions (dict[np.ndarray], optional): true process
                masks. Defaults to None.
            mc_reweight_params (dict, optional): mc reweight
                parameters. Defaults to None.
