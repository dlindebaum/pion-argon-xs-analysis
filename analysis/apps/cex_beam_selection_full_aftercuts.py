...
# ----------------------------- new stuff
# Extend dictionaries for *_after_all_cuts keys
for k in list(x_label.keys()):
    x_label[k + "_after_all_cuts"]   = x_label[k]
    y_scale[k + "_after_all_cuts"]   = y_scale[k]
    x_range[k + "_after_all_cuts"]   = x_range[k]
    nbins[k + "_after_all_cuts"]     = nbins[k]
    ncols[k + "_after_all_cuts"]     = ncols[k]
    truncate[k + "_after_all_cuts"]  = truncate[k]
# ----------------------------- end of new stuff
...
def AnalyseBeamSelection(events : Master.Data, beam_instrumentation : bool, functions : dict, args : dict) -> tuple[dict, pd.DataFrame, dict]:
    ...
    # ----------------------------- new stuff
    # After-all-cuts distributions
    # At this point, events already passed all beam cuts.
    fiducial_cuts = ["TrueFiducialCut", "FiducialStart"]
    ...
    for a in args:
        ...
        _, prop = functions[a](events, **args[a], return_property=True)

        cut_values = args[a].get("cut", None)
        operations = args[a].get("op", None)

        # store as a normal MakeOutput key so MergeSelectionMasks keeps it
        output[f"{a}_after_all_cuts"] = st.MakeOutput(
            prop,
            Tags.GenerateTrueBeamParticleTags(events),
            cut_values,
            operations)
        output[f"{a}_after_all_cuts"]["fs_tags"] = EventSelection.GenerateTrueFinalStateTags(events)
    # ----------------------------- end of new stuff

    df = cut_table.get_table(init_data_name = "no selection", pfos = False, percent_remain = False, relative_percent = False, ave_per_event = False)
    print("OOOOOOOOOO Beam cuts in cut_table:", list(output.keys()))
    return output, df, masks
...
def MakeBeamSelectionPlots(output_mc : dict, output_data : dict, outDir : str, norm : float, book_name = "beam"):
    ...
    with Plots.PlotBook(outDir + f"{book_name}.pdf") as pdf:
        ...
        # This plotting code has been reworked so that the arrows denoting what been kept are only for the pre-cut plots
                ...
                if p == "PiBeamSelection":
                    ...
                    if output_data is None:
                        ...
                        Plots.PlotBarComparision(
                            output_mc[p]["value"], output_data[p]["value"],
                            label_1="MC", label_2="Data",
                            xlabel=x_label[p], fraction=True, barlabel=True
                        )
                    pdf.Save()
                    ...
                    for l in ["linear", "log"]:
                        Plots.PlotTagged(
                            output_mc[p]["value"], output_mc[p]["tags"],
                            data2 = output_data[p]["value"] if output_data else None,
                            norm = norm, y_scale = l, x_label = x_label[p],
                            bins = nbins[p], ncols = ncols[p],
                            x_range = x_range[p], truncate = truncate[p]
                        )

                        # Saftey (not rlly needed)
                        if not p.endswith("_after_all_cuts"):
                    ...
                    for l in ["linear", "log"]:
                        Plots.PlotTagged(
                            output_mc[p]["value"], output_mc[p]["tags"],
                            data2=None, norm=norm, y_scale=l,
                            x_label=x_label[p], bins=nbins[p], ncols=ncols[p],
                            x_range=x_range[p], truncate=truncate[p]
                        )
                        ...
                        if not p.endswith("_after_all_cuts"):
                            ...
                            if face in [">", "<", "left", "right"]:
                                Plots.DrawMultiCutPosition(
                                    output_mc[p]["cuts"], face=face,
                                    arrow_length=st.CalculateArrowLength(output_mc[p]["value"], x_range[p]),
                                    arrow_loc=0.7, color="k"
                                )
                    ...
                    for t in ["BeamScraperCut", "BeamScraperCut_scraper_tag"]:
                        Plots.PlotTagged(
                            output_mc[p]["value"], output_mc[t]["tags"],
                            data2=output_data[p]["value"] if output_data else None,
                            norm=norm, y_scale=y_scale[p], x_label=x_label[p],
                            bins=nbins[p], ncols=ncols[p],
                            x_range=x_range[p], truncate=truncate[p]
                        )

                        # # Safety (not rlly needed)
                        if not p.endswith("_after_all_cuts"):
                            face = output_mc[p].get("op", None)
                            if face in [">", "<", "left", "right"]:
                                Plots.DrawMultiCutPosition(
                    ...
                    if not p.endswith("_after_all_cuts"):
                        ...
                        if face in [">", "<", "left", "right"]:
                            Plots.DrawMultiCutPosition(
                                output_mc[p]["cuts"], face=face,
                                arrow_length=arrow_l, arrow_loc=0.7, color="k"
                            )
...
def main(args):
    ...
    # ------------------ New stuff (replcement)
    if output_mc["beam"]:
        beam_mc = {k: v for k, v in output_mc["beam"]["data"].items()
                if not k.endswith("_after_all_cuts")}
        beam_data = None
        if output_data:
            beam_data = {k: v for k, v in output_data["beam"]["data"].items()
                        if not k.endswith("_after_all_cuts")}
    ...
    # After-all-cuts plots (pull from beam["data"]), not replacemtn anymore
    after_mc = {k: v for k, v in output_mc["beam"]["data"].items()
                if k.endswith("_after_all_cuts")}
    after_mc["final_tags"] = output_mc["beam"]["data"]["final_tags"]

    after_data = None
    if output_data:
        after_data = {k: v for k, v in output_data["beam"]["data"].items()
                    if k.endswith("_after_all_cuts")}
        after_data["final_tags"] = output_data["beam"]["data"]["final_tags"]

    if len(after_mc) > 1:  # more than final_tags
        MakeBeamSelectionPlots(
            after_mc,
            after_data,
            outdir + "plots/",
            norm=args.norm,
            book_name="beam_after"
        )
    else:
        print("No after-all-cuts entries found in beam['data']")
    # ----------------- end of new stuff
