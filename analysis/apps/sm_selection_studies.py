...
def BasicQuantities(events : Master.Data, start_showers_all : ak.Array, to_merge : ak.Array, signal_all : ak.Array, background : ak.Array) -> dict:
    ...
    data = {
        ...
        "energy_signal" : ak.ravel(events.recoParticles.shower_energy[to_merge][signal_all]),
        "energy_background" : ak.ravel(events.recoParticles.shower_energy[to_merge][background]),
        "nHits_collection_signal" : ak.ravel(events.recoParticles.n_hits_collection[to_merge][signal_all]),
        "nHits_collection_background" : ak.ravel(events.recoParticles.n_hits_collection[to_merge][background]),
        "nHits_signal" : ak.ravel(events.recoParticles.n_hits[to_merge][signal_all]),
        "nHits_background" : ak.ravel(events.recoParticles.n_hits[to_merge][background]),
        "cnn_signal" : ak.ravel(events.recoParticles.cnn_score[to_merge][background]),
        "cnn_background" : ak.ravel(events.recoParticles.cnn_score[to_merge][signal_all]),
