...
Author: Shyam Bhuller// aftercut edits by MH
...
# ----------------------------- new stuff
# Extend dictionaries for *_nminus1 keys only
base_keys = list(x_label.keys())
for k in base_keys:
    suff = "_nminus1"
    x_label[k + suff]  = x_label[k]
    y_scale[k + suff]  = y_scale[k]
    x_range[k + suff]  = x_range[k]
    nbins[k + suff]    = nbins[k]
    ncols[k + suff]    = ncols[k]
    truncate[k + suff] = truncate[k]
# ----------------------------- end of new stuff
...
def AnalyseBeamSelection(events : Master.Data, beam_instrumentation : bool, functions : dict, args : dict) -> tuple[dict, pd.DataFrame, dict]:
    """ Manually applies the beam selection while storing the value being cut on, cut values and truth tags in order to do plotting
        ...
        Additionally:
          - *_after_all_cuts: distribution after all beam cuts.
          - *_nminus1: n-1 distribution (all other beam cuts applied, this one removed).
    """

    # Beam-start sample: after fiducial, before any beam cuts
    events_beam_start = events.Filter(returnCopy=True)

    output = {}
    cut_table = Master.CutTable.CutHandler(
        events,
        tags = Tags.GenerateTrueBeamParticleTags(events) if beam_instrumentation is False else None
    )

    output["no_selection"] = st.MakeOutput(
        None,
        Tags.GenerateTrueBeamParticleTags(events),
        None,
        None,
        EventSelection.GenerateTrueFinalStateTags(events)
    )

    fiducial_cuts = ["TrueFiducialCut", "FiducialStart"]
    masks = {k: functions[k](events, **v) for k, v in args.items() if k not in fiducial_cuts}

    #* pi+ beam selection (sequential)
    ...
    output["PiBeamSelection"] = st.MakeOutput(
        counts,
        Tags.GenerateTrueBeamParticleTags(events),
        None,
        None
    ...
    # Other beam cuts (sequential)
    for a in args:
        if a in ["PiBeamSelection"] + fiducial_cuts:
            continue
        ...
        if (a == "BeamScraperCut") and (beam_instrumentation == False):
            ...
            output[a + "_scraper_tag"] = st.MakeOutput(
                None,
                Tags.BeamScraperTag(scraper_id),
                None,
                None
            )
    ...
    #* true particle population (after all cuts)
    output["final_tags"] = st.MakeOutput(
        None,
        Tags.GenerateTrueBeamParticleTags(events),
        None,
        None,
    ...
    # ----------------------------- new n-1 block
    # n-1 = PiBeamSelection AND all other beam cuts except this one,
    # evaluated on the beam-start sample.
    fiducial_cuts = ["TrueFiducialCut", "FiducialStart"]

    # Pi mask on the beam-start sample
    mask_pi_all, _ = BeamParticleSelection.PiBeamSelection(
        events_beam_start,
        **args["PiBeamSelection"],
        return_property=True
    )

    # Full-sample masks for each beam cut (non-fiducial, non-Pi, non-Pandora)
    beam_masks_full = {}
    for a in args:
        if a in ["PiBeamSelection", "PandoraTagCut"] + fiducial_cuts:
            continue
        beam_masks_full[a] = functions[a](events_beam_start, **args[a])

    def and_all(mask_list):
        if len(mask_list) == 0:
            return ak.ones_like(mask_pi_all, dtype=bool)
        m = ak.ones_like(mask_pi_all, dtype=bool)
        for mm in mask_list:
            m = np.logical_and(m, mm)
        return m

    for a, m_a in beam_masks_full.items():
        # all other beam cuts + Pi, excluding a
        other_masks = [mask_pi_all]
        for b, m_b in beam_masks_full.items():
            if b == a:
                continue
            other_masks.append(m_b)
        mask_n1 = and_all(other_masks)

        # Sample and tags for this n-1 definition
        events_n1 = events_beam_start.Filter([mask_n1], [mask_n1], returnCopy=True)
        tags_n1 = Tags.GenerateTrueBeamParticleTags(events_n1)
        fs_tags_n1 = EventSelection.GenerateTrueFinalStateTags(events_n1)

        # Variable being cut on, evaluated on beam-start sample then masked
        _, prop_full = functions[a](events_beam_start, **args[a], return_property=True)
        prop_n1 = prop_full[mask_n1]

        cut_values = args[a].get("cut", None)
        operations = args[a].get("op", None)

        key = f"{a}_nminus1"
        output[key] = st.MakeOutput(
            prop_n1,
            tags_n1,
            cut_values,
            operations
        )
        output[key]["fs_tags"] = fs_tags_n1

    # For sanity, print keys once
    print("Beam output keys:", list(output.keys()))
    # ----------------------------- end n-1 block

    df = cut_table.get_table(
        init_data_name = "no selection",
        pfos = False,
        percent_remain = False,
...
def MakeBeamSelectionPlots(output_mc: dict, output_data: dict, outDir: str, norm: float, book_name="beam"):
    """Beam particle selection plots (n-1 only).

    Args:
        output_mc (dict): MC to plot (per-cut entries + final_tags)
        output_data (dict): data to plot (same keys as MC, or None)
    ...
    with Plots.PlotBook(outDir + f"{book_name}.pdf") as pdf:
        ...
        for p in output_mc:
            ...
            # 3) APA3Cut: always draw cut line(s) if cuts exist
            if p == "APA3Cut":
                for l in ["linear", "log"]:
                    Plots.PlotTagged(
                        output_mc[p]["value"],
                        output_mc[p]["tags"],
                        data2=output_data[p]["value"] if output_data else None,
                        norm=norm,
                        y_scale=l,
                        x_label=x_label[p],
                        bins=nbins[p],
                    ...
                    cuts = output_mc[p].get("cuts", None)
                    face = output_mc[p].get("op", None)
                    if cuts is not None:
                        Plots.DrawMultiCutPosition(
                            cuts,
                            face=face,
                            arrow_length=st.CalculateArrowLength(output_mc[p]["value"], x_range[p]),
                            arrow_loc=0.7,
                            color="k",
                        )
                    pdf.Save()
            ...
            # 4) TrueFiducialCut: always draw cut line(s) if cuts exist
            if p == "TrueFiducialCut":
                for l in ["linear", "log"]:
                    Plots.PlotTagged(
                        output_mc[p]["value"],
                        output_mc[p]["tags"],
            ...
            # 5) BeamScraperCut: always draw cut line(s) if cuts exist
            if p == "BeamScraperCut":
            ...
            # 6) Generic numeric cuts (TrackScoreCut, NHitsCut, DxyCut, DzCut, CosThetaCut, etc.)
            scale = y_scale[p] if y_scale[p] is not None else "linear"
            ...
            face = output_mc[p].get("op", None)
            if cuts is not None:
                arrow_l = 0.02 if p == "CosThetaCut" else st.CalculateArrowLength(output_mc[p]["value"], x_range[p])
                Plots.DrawMultiCutPosition(
                    cuts,
                    face=face,
        ...
        # Final truth composition after all cuts (still useful for these studies)
        if "final_tags" in output_mc:
...
def main(args):
    ...
    # ------------------ Beam n-1 plots (replace sequential)
    if output_mc["beam"]:
        # n-1 keys from beam["data"]
        beam_mc = {
            k.replace("_nminus1", ""): v
            for k, v in output_mc["beam"]["data"].items()
            if k.endswith("_nminus1")
        }

        # Always add final_tags, needed by MakeBeamSelectionPlots
        beam_mc["final_tags"] = output_mc["beam"]["data"]["final_tags"]

        beam_data = None
        if output_data:
            beam_data = {
                k.replace("_nminus1", ""): v
                for k, v in output_data["beam"]["data"].items()
                if k.endswith("_nminus1")
            }
            beam_data["final_tags"] = output_data["beam"]["data"]["final_tags"]

        print("Beam n-1 keys used for plotting:", list(beam_mc.keys()))

        MakeBeamSelectionPlots(
            beam_mc,
            beam_data,
            outdir + "plots/",
            norm=args.norm,
            book_name="beam"   # beam.pdf is now n-1
        )
    # ------------------ end of n-1 block
