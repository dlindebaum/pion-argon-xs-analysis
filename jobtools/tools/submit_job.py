...
def ParseFile(file : str, fcl : str, outdir : str, file_list : str):
    f = open(file, "r")

    replacement = ""
    for line in f:
        if "FHICL=" in line and fcl != "":
            print("fhicl file variable found")
            replacement += f"FHICL=\"{fcl}\"" + "\n"
            continue
#        if "N_EVENTS=" in line and n in range(1, 1000):
#            print("nEvents variable found")
#            replacement += f"N_EVENTS={n}" + "\n"
#            continue
        if "OUTDIR_NAME=" in line and outdir != "":
            print("nEvents variable found")
            replacement += f"OUTDIR_NAME=\"{outdir}\"" + "\n"
            continue
        if "FILE_LIST=" in line and file_list != "":
            print("file list name variable found")
            replacement += f"FILE_LIST=\"{file_list}\"" + "\n"
            continue
        replacement += line
...
def ConfigureScript(fcl : str, outdir : str, file_list : str):
    ...
    exit 0
    '''
>>>>>>> added new scripts to help run jobs, cleaned up jobtools directory and updated readme
...
=======
    ConfigureScript(args.fhicl, args.outDir, args.file_list)

    bashCommand = "jobsub_submit --mail_never -G dune --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC,OFFSITE -l '+SingularityImage=\"/cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7:latest\"' --lines '+FERMIHTC_AutoRelease=True' --lines '+FERMIHTC_GraceMemory=1024' --lines '+FERMIHTC_GraceLifetime=1800' --append_condor_requirements='(TARGET.HAS_Singularity==true&&TARGET.HAS_CVMFS_dune_opensciencegrid_org==true&&TARGET.HAS_CVMFS_larsoft_opensciencegrid_org==true&&TARGET.CVMFS_dune_opensciencegrid_org_REVISION>=1105&&TARGET.HAS_CVMFS_fifeuser1_opensciencegrid_org==true&&TARGET.HAS_CVMFS_fifeuser2_opensciencegrid_org==true&&TARGET.HAS_CVMFS_fifeuser3_opensciencegrid_org==true&&TARGET.HAS_CVMFS_fifeuser4_opensciencegrid_org==true)' "

    bashCommand += f"-N {args.nJobs} " # 1
    bashCommand += f"--memory={args.memory} " # 2800MB
    bashCommand += f"--disk={args.disk} " # 10GB
    bashCommand += f"--expected-lifetime={args.lifetime} " # 3h
    bashCommand += f"--cpu={args.cpu} " # 1
    bashCommand += f"--tar_file_name=dropbox://{args.tar_file} "
    bashCommand += f"file:///tmp/job_script.sh"
...
=======
    parser.add_argument("-n", "--number-of-jobs", dest="nJobs", type=int, default=1, help="number of jobs to submit")
>>>>>>> added new scripts to help run jobs, cleaned up jobtools directory and updated readme
...
=======
    parser.add_argument("-t", "--tarball", dest="tar_file", type=str, default="", help="custom tarball to use")
    parser.add_argument("-f", "--fhicl", dest="fhicl", type=str, default="", help="fhicl file to run")
    #parser.add_argument("-e", "--number-of-events", dest="nEvents", type=int, default="10", help="number of events to process per job")
    parser.add_argument("-o", "--outdir-name", dest="outDir", type=str, default="output", help="name of output directory (will automatically create in scratch directory)")
    parser.add_argument("-i", "--input-file-list", dest="file_list", type=str, default="file_list.txt", help="file list name (should be in the tarfile!)")
    args = parser.parse_args()
    main(args)
>>>>>>> added new scripts to help run jobs, cleaned up jobtools directory and updated readme
